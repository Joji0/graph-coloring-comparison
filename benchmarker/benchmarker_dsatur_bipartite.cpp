...
const string DATA_DIR = "../data/bipartite/";
...
int main() {
    ...
    string inputFile = DATA_DIR + "bipartite_n" + to_string(n) + ".txt";
    string outputFile =
        RESULT_DIR + "dsatur_bipartite_n" + to_string(n) + "_results.txt";

    Graph G = loadGraphFromFile(inputFile);
    int vertexCount = G.size() - 1;
    ...
    cout << "\n▶️ Running DSatur on bipartite graph with n = " << n << "...\n";

    for (int i = 1; i <= 1001; ++i) {
        vector<int> coloring;

        auto start = high_resolution_clock::now();
        coloring = dsaturColoring(G, vertexCount);
