...
const string DATA_DIR = "../data/grid/";
...
int main() {
    ...
    cout << "📌 Choose grid size (available: 5, 25, 100): ";
    cin >> n;

    if (n != 5 && n != 25 && n != 100) {
        cerr << "❌ Invalid grid size. Allowed values: 5, 25, 100\n";
        return 1;
    }

    filesystem::create_directory(RESULT_DIR);

    string inputFile = DATA_DIR + "grid_n" + to_string(n) + ".txt";
    string outputFile =
        RESULT_DIR + "backtracking_grid_n" + to_string(n) + "_results.txt";
    ...
    cout << "\n▶️ Running backtracking on grid graph with n = " << n << "...\n";
