...
const string DATA_DIR = "../data/crown/";
...
int main() {
    ...
    cout << "📌 Choose amount of vertices (original crown size): ";
    cin >> n;
    ...
    string inputFile = DATA_DIR + "crown_n" + to_string(n) + ".txt";
    string outputFile =
        RESULT_DIR + "greedy_crown_n" + to_string(n) + "_results.txt";
    ...
    cout << "\n▶️ Running greedy on crown graph with 2n = " << 2 * n << "...\n";

    for (int i = 1; i <= 1001; ++i) {
        auto start = high_resolution_clock::now();
        vector<int> coloring = greedyColoring(G, (int)G.size() - 1);
        ...
        out << "Iteration " << i << ": colors used = " << maxColor
            << ", time = " << duration << " ns"
            << (i > 1 ? "\n" : " (warmup is ignored)\n");

        cout << "⏱️ Iteration " << i << ": colors used = " << maxColor
             << ", time = " << duration << " ns"
             << (i > 1 ? "\n" : " (warmup is ignored)\n");
    }
